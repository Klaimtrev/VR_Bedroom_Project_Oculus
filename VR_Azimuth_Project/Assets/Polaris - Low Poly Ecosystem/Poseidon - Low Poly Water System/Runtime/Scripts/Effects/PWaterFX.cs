...
namespace Pinwheel.Poseidon.FX
{
    ...
    public class PWaterFX : MonoBehaviour, ISerializationCallbackReceiver
    {
        public const string UNDERWATER_SHADER_BRP = "Hidden/Poseidon/Underwater";
        public const string UNDERWATER_SHADER_URP = "Hidden/Poseidon/UnderwaterURP";
        public const string WETLENS_SHADER_BRP = "Hidden/Poseidon/WetLens";
        public const string WETLENS_SHADER_URP = "Hidden/Poseidon/WetLensURP";
        ...
        [SerializeField]
        private Material cacheUnderwaterMaterial;
        [SerializeField]
        private Material cacheWetLensMaterial;
        ...
        public void CacheFxMaterialForSerializing()
        {
            cacheUnderwaterMaterial = null;
            cacheWetLensMaterial = null;
            ...
            if (Profile != null)
            {
                if (PCommon.CurrentRenderPipeline == PRenderPipelineType.Builtin)
                {
                    ...
                    if (Profile.EnableUnderwater)
                    {
                        cacheUnderwaterMaterial = new Material(Shader.Find(UNDERWATER_SHADER_BRP));
                    }
                    if (Profile.EnableWetLens)
                    {
                        cacheWetLensMaterial = new Material(Shader.Find(WETLENS_SHADER_BRP));
                    }
#endif
                }
                else if (PCommon.CurrentRenderPipeline == PRenderPipelineType.Universal)
                {
                    ...
                    if (Profile.EnableUnderwater)
                    {
                        cacheUnderwaterMaterial = new Material(Shader.Find(UNDERWATER_SHADER_URP));
                    }
                    if (Profile.EnableWetLens)
                    {
                        cacheWetLensMaterial = new Material(Shader.Find(WETLENS_SHADER_URP));
                    }
#endif
                }

                if (cacheUnderwaterMaterial != null)
                {
                    if (Profile.UnderwaterEnableCaustic)
                    {
                        cacheUnderwaterMaterial.EnableKeyword(PMat.KW_PP_CAUSTIC);
                    }
                    else
                    {
                        cacheUnderwaterMaterial.DisableKeyword(PMat.KW_PP_CAUSTIC);
                    }

                    if (Profile.UnderwaterEnableDistortion)
                    {
                        cacheUnderwaterMaterial.EnableKeyword(PMat.KW_PP_DISTORTION);
                    }
                    else
                    {
                        cacheUnderwaterMaterial.DisableKeyword(PMat.KW_PP_DISTORTION);
                    }
                }
            }
        }
